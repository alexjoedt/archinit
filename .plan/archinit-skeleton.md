...
│   ├── install.sh update.sh doctor.sh tui.sh list.sh version.sh snapshot.sh restore.sh
├── modules/                    # idempotent units, numbered
│   ├── 00-base/ 10-aur-helper/ 20-services/ 30-desktop/ 40-dev/  (each: module.sh)
├── config/
│   ├── defaults.conf
│   └── packages/
...
```
Dispatcher loops modules numerically; `install` skips when `module_check` returns 0 unless `--force`.
...
- **File(s)**: `lib/core.sh`
- **Details**: `set -euo pipefail`; safe `IFS`; double-source guard (`[[ -n ${_ARCHINIT_CORE:-} ]] && return; _ARCHINIT_CORE=1`); define `ARCHINIT_HOME` (default `${ARCHINIT_HOME:-$HOME/.archinit}`); `die()`, `require_cmd()`, `is_root()`, `as_root()` (wraps `sudo` only when not root); `ERR` trap printing failing command + line via `BASH_COMMAND`/`LINENO`. Provide `DRY_RUN`, `ASSUME_YES`, `VERBOSE`, `QUIET`, `NO_COLOR` globals defaulting to empty. `run()` helper that echoes (dry-run) or executes a command.
- **Verify**: `bash -n lib/core.sh`; `shellcheck lib/core.sh`

#### 1.2 log.sh — leveled logging + color
- [ ] **Task**: Logging helpers respecting `--quiet/--verbose/--no-color`
- **File(s)**: `lib/log.sh`
- **Details**: `log_info/log_warn/log_error/log_debug/log_ok`. Colors via tput, disabled when `NO_COLOR` set or stdout not a tty. `log_debug` only when `VERBOSE`. Errors/warns go to stderr.
- **Verify**: `shellcheck`; manual: `VERBOSE=1 ... log_debug` prints, default hides it
...
- **Verify**: `shellcheck`; `--dry-run` prints the exact pacman/yay commands without executing
...
- **Details**: Keys: `UPDATE_INTERVAL_HOURS=24`, `AUR_HELPER=yay`, `DEFAULT_MODULES="base aur-helper services"`, `DOTFILES_MANAGER=dman`, `DOTFILES_REPO=""`, `DOTFILES_PROFILE=default`, `INSTALL_SNAPSHOT_PACKAGES=true`, `DISPLAY_MANAGER=sddm`, `GIT_REMOTE`, branch. `config.sh` loads defaults then sources `config.local` if present.
...
#### 3.5 40-dev (dotfiles via dman)
- [ ] **Task**: AUR dev tools + dotfile application via dman
- **File(s)**: `modules/40-dev/module.sh`
- **Details**: depends on aur-helper; `pkg_install_list aur` (includes `dman`); then apply dotfiles through the dotfiles abstraction in `lib/dotfiles.sh` (default backend `dman`). `module_check` = `dman` present and config initialized (`~/.config/dman/dman.json` exists). GitHub token prompt (from original `archinit.sh` intent) handled via `ui.sh`, used to build the authenticated dotfiles repo URL for `dman init`, stored to a gitignored local file (`$ARCHINIT_HOME/secrets.local`, chmod 600), never logged.
- **Verify**: idempotent; `dman apply` re-run converges (copies only changed files); token not echoed

#### 3.6 lib/dotfiles.sh (dotfiles abstraction)
...
- **File(s)**: `bin/archinit`
- **Details**: Resolve `ARCHINIT_HOME` from script path; source `lib/core.sh log.sh config.sh` + needed libs. Parse global flags (`--dry-run --verbose --quiet --yes --no-color -h/--help`) into core globals. First non-flag arg = verb; default verb (no args) = `tui`. Unknown verb → help + exit 1. Source `cmd/$verb.sh` and call `cmd_<verb> "$@"`.
- **Verify**: `bin/archinit --help`; `bin/archinit version`; unknown verb errors
...
- **Details**: Args = module names (default `config_get DEFAULT_MODULES` or all). For each: source in subshell, `module_check` → skip unless `--force`, else `module_install`; `state_mark` on success. Respect `--dry-run`. Enforce dependency: aur-class auto-requires `aur-helper` (install it first or die with hint). When `INSTALL_SNAPSHOT_PACKAGES=true` and a `latest` snapshot exists, after modules also ensure snapshot packages: `pkg_install_official $(snapshot_native_packages)` + `pkg_install_aur $(snapshot_foreign_packages)` (merged/de-duped with curated lists, idempotent via `--needed`). Skippable with `--no-snapshot`.
- **Verify**: `archinit install base` works; re-run skips; snapshot packages restored when present; `--dry-run` executes nothing
...
- **File(s)**: `install.sh`
- **Details**: `set -euo pipefail`; detect Arch; ensure `git` (`pacman -S --needed --noconfirm git`); if `~/.archinit/.git` exists → `git pull --ff-only`, else `git clone $REPO ~/.archinit`; `chmod +x bin/archinit`; add hook line `source "$HOME/.archinit/shell/archinit.sh"` to `~/.zshrc`/`~/.bashrc` only if absent; print next steps; optionally exec TUI on first run. Re-running must be safe.
- **Verify**: run twice → second is update, no duplicate rc lines
...
#### 6.4 CI (optional)
- [ ] **Task**: GitHub Actions lint + bats
- **File(s)**: `.github/workflows/ci.yml`
- **Details**: Ubuntu runner; install shellcheck/shfmt/bats; run `tests/lint.sh` and `bats`.
